...
namespace mudock {
  ...
  struct vinardo_score: public scoring<queue_type> {
    ...
    vinardo_score(std::shared_ptr<scratchpad<queue_type>> _scratch, //Here I should inser the data that changes between batch
    ...
    // This is only a local memory estimate. Protein-ligand pair memory depends on
    // protein_atoms, which is not available in this static API. The actual buffers
    // are allocated in prepare(), where protein size is known.
    static std::size_t get_private_ligand_mem(const int max_atoms, const knobs) {
      ...
      mem += sizeof(int) * max_atoms;     // pl_protein_atom_idx, protein size not available here
      mem += sizeof(int) * max_atoms;     // pl_ligand_atom_idx
      mem += sizeof(fp_type) * max_atoms; // pl_radius_sum
      mem += sizeof(std::uint8_t) * max_atoms; // pl_hydrophobic_possible
      mem += sizeof(std::uint8_t) * max_atoms; // pl_hbond_possible
    ...
    // TODO: make Vinardo batch sizing able to retrieve the prointe size in some manner
    // get_ligand_mem() currently sees only ligand max_atoms, but Vinardo PL buffers
    // depends on protein_atoms * ligand_atoms, so we conservatively cap batches to 1 until
    // i cannot give a precise estimation
    static batch_multiple get_batch_size(const int,
                                       std::shared_ptr<queue_type>,
                                       const knobs&,
                                       const size_t) {
      return batch_multiple{1, 1};
    }
  private:
    int batch_ligands = 0; //Ligands in the batch
    int batch_atoms = 0; //Atoms in the batch

    //Proteing-Ligand Batch Data
