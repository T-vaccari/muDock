...
namespace mudock {
   // Convert muDock AutoDock atom types into Vinardo atom types in two passes:
  ...
  using atoms_array_type = typename container_aliases::template atoms_size<T>;
   ...
   [[nodiscard]] inline vinardo_atom_type adjust_vinardo_type(const auto& atom_vinardo_type, const auto& graph,   std::size_t atom_idx){
      bool hbonded = false;
   ...
   [[nodiscard]] inline const VinardoTypeInfo& get_vinardo_info(const vinardo_atom_type type) {
