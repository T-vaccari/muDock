...
namespace mudock {
	...
	// the mobility matrix based on the parsed tree strucutre
	// the num of torsional degrees that are used in the affinity function, in which are used to normalize the result

	void mark_not_variable(std::vector<std::uint8_t>& variable, const std::size_t num_atoms, const std::size_t i, const std::size_t j) {
		variable[i * num_atoms + j] = 0;
		variable[j * num_atoms + i] = 0;
	}

	void apply_smina_fixed_marks(const pdbqt_torsion_branch_node& node, std::vector<std::uint8_t>& variable, const std::size_t num_atoms) {
	...
	std::vector<std::uint8_t> build_smina_mobility_matrix(const static_molecule& ligand, const pdbqt_torsion_tree& tree){
	...
	unsigned smina_num_tors(const static_molecule& ligand, std::span<const pdbqt_rotor> rotors, std::span<const vinardo_atom_type> ligand_types) {
		...
		//and they have both at leat 2 bonded heavy atoms 
