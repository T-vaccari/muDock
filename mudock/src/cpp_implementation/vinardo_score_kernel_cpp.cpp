...
namespace mudock {
  void calc_vinardo_energy(  const int batch_atoms,
    ...
    for (int ligand_index{0}; ligand_index < batch_ligands; ++ligand_index) {
      ...
      std::vector<fp_type> inter_scores(scores_per_ligand, fp_type{0});
      std::vector<fp_type> intra_scores(scores_per_ligand, fp_type{0});
      ...
      for (int pose_index{0}; pose_index < scores_per_ligand; ++pose_index) {
        ...
        for (int pair_index{0}; pair_index < pl_count; ++pair_index) {
          ...
          const fp_type distance = sqrt(dx * dx + dy * dy + dz * dz);
        ...
        inter_scores[pose_index] = pl_score;
        intra_scores[pose_index] = ll_score;
      ...
      const fp_type reference_intra_score = intra_scores[best_pose_index];

      for (int pose_index{0}; pose_index < scores_per_ligand; ++pose_index) {
        //Score pre affinity(as in the paper we correct with the reference intra score term)
        const fp_type corrected_score =
            pose_index == best_pose_index
                ? inter_scores[pose_index]
                : inter_scores[pose_index] + intra_scores[pose_index] - reference_intra_score;
        ...
        scores_b[ligand_index * scores_per_ligand + pose_index] = vinardo_affinity(corrected_score, vinardo_num_tors_b[ligand_index]);
      }
  }
